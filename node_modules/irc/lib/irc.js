...
Client.prototype.disconnect = function(message, callback) {
    ...
    message = message || 'Normal disconnecting';
    var self = this;
...
Client.prototype.say = function(target, text, singleLine) {
    if (typeof singleLine != "undefined") {
        this._speak('PRIVMSG', target, text, singleLine);
    } else {
...
Client.prototype._speak = function(kind, target, text, singleLine) {
    var self = this;
    var maxLength = this.maxLineLength - target.length;
    if (typeof text !== 'undefined') {
        var toSay = text.toString().split(/\r?\n/).filter(function(line) {
            return line.length > 0;
        })

        if (typeof singleLine == "undefined") {
            toSay.forEach(function(line) {
        ...
        if (typeof singleLine != "undefined") {
            if (singleLine == 1) {
                self.send(kind, target, linesToSend[0]);
                if (kind == 'PRIVMSG') {
                    self.emit('selfMessage', target, linesToSend[0]);
                }
                return;
            } else if (singleLine == 2) {
                var toSend = linesToSend[0].slice(0, linesToSend[0].length - 3);
                if (toSend == "") {
                    return;
                }
